...
public extension SARSAAgent {
    ...
    */
    func learn(episodes: [Episode]) -> Self {
        return learn(episodes: episodes, alpha: 0.5, gamma: 0.5)
    }
    ...
    - returns: A learned policy based on Q values generated by the SARSA algorithm
    */
    func learn(episodes: [Episode], alpha: Float, gamma: Float) -> Self {
        return episodes.reduce(self) { (result, entry) -> Self in
            return result.learn(episode: entry, alpha: alpha, gamma: gamma)
        }
    }
}

extension SARSAAgent {
    func learn(episode: Episode, alpha: Float, gamma: Float) -> Self {
        guard let firstStateActionPair = episode.first else { return self }
        return episode.dropFirst().reduce((self, firstStateActionPair)) { (result, entry) -> (Self, StateActionPair) in
            return (result.0.learn(sa1: result.1, sa2: entry, alpha: alpha, gamma: gamma), entry)
        }.0
    }
    
    func learn(sa1: StateActionPair, sa2: StateActionPair, alpha: Float, gamma: Float) -> Self {
        let r = reward(s1: sa1.0, s2: sa2.0)
        let q1 = policy.qValue(for: sa1.0, action: sa1.1) ?? 0.0
        let q2 = policy.qValue(for: sa2.0, action: sa2.1) ?? 0.0
        let newQ = q1 + (alpha * (r + (gamma * q2) - q1))
        return update(policy: policy.changing(state: sa1.0, action: sa1.1, to: newQ)).visit(state: sa1.0).visit(state: sa2.0)
    }
}
